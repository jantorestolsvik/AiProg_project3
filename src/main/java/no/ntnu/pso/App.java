...
public class App 
{
	...
	{   
		...
		System.out.println("Which problem do you want? (1:Roost, 2:Knapsack)");


		int problem = in.nextInt();
		try {
			if (problem < 1||problem>2) {
				throw new Exception("1 and 2 are the only accseptable answeres.");
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(1);
		...
		int volume=0;
		if(problem==2){
			System.out.println("Add volume constreints? (1:yes, 2:no)");
			try {
				volume = in.nextInt();
				if (volume < 1) {
					throw new Exception("1 and 2 are the only accseptable answeres.");
			...
			try {
				nrOfRuns = in.nextInt();
				if (nrOfRuns < 1) {
					throw new Exception("Nr of iterations must be 1 or above");
				}
			} catch (Exception e) {
				e.printStackTrace();
				System.exit(1);
        ...
        System.out.println("Boid positions:");*/
			...
			if(volume==1)
				kp.run(nrOfBoids, nrOfRuns, true);
			else
				kp.run(nrOfBoids, nrOfRuns, false);
		}
	}
